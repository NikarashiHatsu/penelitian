<x-app-layout>
    ...
    <div class="card mb-3">
        <div class="card-body">
            <form action="{{ route('dashboard.gallery.update', $gallery) }}" method="post" enctype="multipart/form-data" x-data="{ isDescription: '{{ $gallery->type == 'Description' ? true : false }}' }">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-sm-12 col-sm-6 col-md-4 col-lg-3">
                        <x-tabler::form.input.select
                            ...
                            required
                        >
                            <option {{ $gallery->type == "Image" ? "selected" : "" }} value="Image">Foto</option>
                            <option {{ $gallery->type == "Description" ? "selected" : "" }} value="Description">Deskripsi</option>
                        </x-tabler::form.input.select>
                    </div>
